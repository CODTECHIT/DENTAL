      ...
      <div className="max-w-7xl mx-auto px-4 py-2.5 flex items-center justify-between gap-6">
        <div className="flex items-center gap-6">
          ...
          <a
            ...
            className="flex items-center gap-2 hover:text-gold transition-colors"
          >
            <MessageCircle className="w-3.5 h-3.5 text-gold" /> {PHONE}
          </a>
          <a
            href={`mailto:${EMAIL}`}
            className="flex items-center gap-2 hover:text-gold transition-colors"
        ...
        <div className="flex items-center gap-2">
          <MapPin className="w-3.5 h-3.5 text-gold" /> {ADDRESS_SHORT}
        </div>
      </div>
    </div>
  );
...
export function Header() {
  ...
  return (
    <>
      <TopBar />
      <header
        className={`sticky top-0 z-40 transition-all rounded-b-2xl lg:rounded-b-3xl border-b-[3px] border-gold shadow-md ${scrolled ? "bg-white/95 backdrop-blur" : "bg-white"}`}
      >
        <div className="max-w-7xl mx-auto px-4 py-1 md:py-3 flex items-center justify-between">
          <Link to="/" className="flex items-center justify-start gap-3 z-10">
            <img
              src="/logo.jpg"
              alt="Sri Shobh Dental Logo"
              className="h-[50px] sm:h-[58px] lg:h-[72px] w-auto object-contain shrink-0"
            />
          </Link>
          ...
          <div className="flex items-center justify-end gap-2 justify-self-end lg:justify-self-auto">
            ...
            <button
              ...
              className="lg:hidden p-2.5 rounded-xl border-[2px] border-gold bg-ink text-gold hover:bg-ink/90 transition-colors shadow-sm flex items-center justify-center"
            >
              {open ? (
